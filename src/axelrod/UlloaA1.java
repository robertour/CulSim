 ...
 * 5. Also, the possibility to go back to its own culture 
 * @author tico
 *
 */
public class UlloaA1 extends Ulloa1 {
			...
			for (int ic = 0; ic < CHECKPOINT; ic++) {
				for (int i = 0; i < TOTAL_AGENTS; i++) {
					...
					if (/*agents_overlap != FEATURES // this is avoiding cultural change!!
						...
						boolean is_cultural_resilience = rand.nextFloat() <= cultural_overlap / 
								// Math.max because it might have been a selection error
								(float) (Math.max(1, agents_overlap) +  
										cultural_overlap);
						
						// if the culture roll against the change, it should roll again to give the
						// trace back to the agent
						if (is_cultural_resilience){
							if (rand.nextFloat() <= 1 - ((float) cultural_overlap / (float) FEATURES)) {
