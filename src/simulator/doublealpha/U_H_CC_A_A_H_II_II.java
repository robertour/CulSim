 ...
 *
 */
public class U_H_CC_A_A_H_II_II extends Ulloa1 {
			...
			for (int ic = 0; ic < CHECKPOINT; ic++) {
				for (int i = 0; i < TOTAL_AGENTS; i++) {
					...
					if (rand.nextFloat() < prob_int ) {
						...
						double cultural_resistance = cultural_overlap * ALPHA;
												
						
						if (selected_trait == nationality_trait || beliefs[r][c][selected_feature] != nationality_trait || 
							...
							// accept the change
								...
								if (neighbors_cultural_overlap > cultural_overlap) {
