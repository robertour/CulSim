 ...
 *
 */
public class U_H_CC_A_A__H_I_II extends Ulloa1 {
			...
			for (int ic = 0; ic < CHECKPOINT; ic++) {
				for (int i = 0; i < TOTAL_AGENTS; i++) {
					...
					if (rand.nextFloat() < prob_int ) {
						...
						double cultural_resilience = 1 - Math.pow ( 1- (cultural_overlap / (float) FEATURES), ALPHA);
						
						//double cultural_resilience = 1/(Math.exp(-10*(((cultural_overlap / (float) FEATURES))-0.5)) + 1);
						
						if (rand.nextFloat() >= cultural_resilience ) {
