package deprecated.simulator.scenarios;
 ...
 *
 */
public class ScenarioC extends Ulloa1 {
				...
				for (int i = 0; i < TOTAL_AGENTS; i++) {
					...
					if (rand.nextFloat() <  
						...
						if (selected_trait == institution_trait || 
							...
							// when the agent doesn't have any similarity with the cultures then his identity 
							// and accept the trait. It also joins the neighbor's institution.  This also avoid 
							// divisions by 0 in the next condition.
							if (institution_overlap == 0 && neighbors_institution_overlap == 0) {
								...
								institution_resistance = institution_overlap * ALPHA;
								
							    if (// Institutional resilience: resistance to change based on institution 
									// similarity and neighbor's institution similarity
									rand.nextFloat() >= institution_resistance / 
									(float) neighbors_institution_overlap * BETA +  
							...
							} // END of else (avoid division by zero)
